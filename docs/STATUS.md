...
- [ ] **Windows Task Scheduler — `sync-all` (EIN Task für ALLES)**: **Admin**-CMD → `schtasks /create /tn "Personal-OS-Sync" /xml "C:\Users\Administrator\Documents\Claude\Personal OS\scripts\sync-all-task.xml" /f`. Führt bei jeder Anmeldung (1 Min Delay) **alle 5 Schritte** aus: Garmin→Obsidian, _Eingang-Ingest, Storage→Obsidian, Logbuch-Nachbau, Knowledge-Nachbau. **Ersetzt** die 3 alten Einzel-Tasks (Eingang-Ingest, Garmin-Obsidian-Sync, Supabase-Obsidian-Sync) und den separaten Logbuch-Task — diese kannst du danach deaktivieren (`schtasks /change /tn "Eingang-Ingest" /disable` etc.), sonst laufen Schritte doppelt (ist idempotent, also unkritisch).
