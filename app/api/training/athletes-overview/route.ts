...
export async function GET() {
  ...
  const athletes = await Promise.all(persons.map(async (p) => {
    ...
    const [{ data: firstSession }, { data: sessions }, { data: training }, { data: lastAct }] = await Promise.all([
      supabaseAdmin.from('training_plan_sessions').select('date').eq('user_id', pid)
        .order('date', { ascending: true }).limit(1).maybeSingle(),
      supabaseAdmin.from('training_plan_sessions').select('date, completed_at, is_optional')
    ...
    for (const s of sessions ?? []) {
      ...
      weekMap[wk].planned++
      if (s.completed_at) weekMap[wk].done++
    }
